...
int main(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (argument == "--target" && i + 1 < argc) target = argv[++i];
        else if (argument == "--source" && i + 1 < argc) source_path = argv[++i];
        else if (argument == "--root" && i + 1 < argc) root = argv[++i];
        else if (argument == "--output" && i + 1 < argc) output_path = argv[++i];
        else if (argument == "--config" && i + 1 < argc) config_storage.emplace_back(argv[++i]);
        else if (argument == "--workload" && i + 1 < argc) workload.push_back(std::stoll(argv[++i]));
        else {
    ...
    write_file(std::filesystem::path(output_path) / "final.loom", result.final_module_text, result.final_module_text_size);

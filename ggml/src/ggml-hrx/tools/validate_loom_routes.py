...
import validate_hsaco_routes as hsaco
...
def validate_metadata(source_root):
    ...
    metadata = hsaco.read_json(metadata_path)
    hsaco.unknown_fields(metadata, METADATA_FIELDS, metadata_path)
    if hsaco.require_string(metadata, "schema", metadata_path) != CATALOG_SCHEMA_V0:
        raise ValueError(f"{metadata_path}: unsupported schema")
    hsaco.require_int(metadata, "version", metadata_path)
    for target in hsaco.require_list(metadata, "targets", metadata_path):
...
def validate_definition(definition, definition_path, source_root):
    hsaco.unknown_fields(definition, DEFINITION_FIELDS, definition_path)
    schema = hsaco.require_string(definition, "schema", definition_path)
    if schema != DEFINITION_SCHEMA_V1:
        raise ValueError(f"{definition_path}: unsupported definition schema {schema}")
    source_format = hsaco.require_string(definition, "source_format", definition_path)
    ...
    hsaco.validate_definition(definition, definition_path, source_root)
...
def load_definitions(source_root):
    ...
    for definition_path in sorted(definitions_dir.glob("*.json")):
        definition = hsaco.read_json(definition_path)
        validate_definition(definition, definition_path, source_root)
        definition_id = hsaco.require_string(definition, "id", definition_path)
...
def validate_config(route, route_path, context):
    config = hsaco.require_dict(route, "config", route_path)
    hsaco.unknown_fields(config, CONFIG_FIELDS, f"{route_path}: config")
    mode = hsaco.require_string(config, "mode", f"{route_path}: config")
    if mode != "compile":
        raise ValueError(f"{route_path}: config.mode must be compile")
    bindings = hsaco.require_array(config, "bindings", f"{route_path}: config")
    ...
    for i, binding in enumerate(bindings):
        ...
        hsaco.unknown_fields(binding, CONFIG_BINDING_FIELDS, source)

        name = hsaco.require_string(binding, "name", source)
        ...
        binding_type = hsaco.require_string(binding, "type", source)
        if binding_type not in hsaco.SCALAR_TYPES:
            supported = ", ".join(sorted(hsaco.SCALAR_TYPES))
        ...
        if has_source:
            value_type = context.resolve_source(hsaco.require_string(binding, "source", source), f"{source}.source")
            ...
            hsaco.validate_literal(binding["value"], binding_type, f"{source}.value")
...
def validate_route(route_path, definitions):
    ...
    hsaco.unknown_fields(route, TOP_LEVEL_FIELDS, route_path)
    schema = hsaco.require_string(route, "schema", route_path)
    if schema != ROUTE_SCHEMA_V1:
        raise ValueError(f"{route_path}: unsupported route schema {schema}")
    hsaco.require_string(route, "id", route_path)
    route_format = hsaco.require_string(route, "format", route_path)
    if route_format != ROUTE_FORMAT:
        raise ValueError(f"{route_path}: unsupported route format {route_format}")
    hsaco.require_int(route, "priority", route_path)
    if "tests" in route:
        hsaco.require_dict(route, "tests", route_path)
    ...
    op_rule, tensors, attributes, predicates = hsaco.validate_match(route, route_path, definition)
    derived = hsaco.require_dict(route, "derived", route_path)
    context = hsaco.RouteContext(route_path, op_rule, tensors, attributes, derived)
    hsaco.validate_derived(route, route_path, context)
    hsaco.validate_predicates(predicates, route_path, context)
    validate_config(route, route_path, context)
    hsaco.validate_invocation(route, route_path, definition, context)
...
def validate_catalog(source_root):
    ...
    route_names = hsaco.require_list(metadata, "routes", metadata_path)
